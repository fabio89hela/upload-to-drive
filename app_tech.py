...
if mode == "Carica un file audio":
    ...
    with st.expander("Sezione 1"):
        ...
        while True:
            i=i+1
            _ = st_javascript("""localStorage.getItem('combined_transcriptions');""",key="trascrizione_testo"+str(i)) 
            transcription_text = _ if _ is not None else ""
            _ = st_javascript("""localStorage.getItem('update_time');""",key="tempo_trascr"+str(i)) 
            timestamp = _ if _ is not None else ""
